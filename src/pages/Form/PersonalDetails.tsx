/**
 * Personal Details: resident details and contact preferences.
 *
 * This step collects the resident's name plus any optional personal, contact, and address details
 * before moving on to the actions step.
 */
...
import type { ContactMethod, FormData } from "./model/formFieldTypes";
import {
...
export default function PersonalDetails() {
  ...
  const COPY = {
    ...
    personalDetails: {
      ...
      tts: "Personal details. First name and last name are required. Pronouns, preferred name, middle name, and date of birth are optional.",
    },
    contactDetails: {
  ...
  const firstNameMissing = formData.firstName.trim() === "";
  const lastNameMissing = formData.lastName.trim() === "";
  const postcodeInvalid = postcodeRaw.trim() !== "" && !isValidUkPostcode(postcodeRaw);

  const emailRaw = (formData.email ?? "").trim();
  const emailInvalid = emailRaw !== "" && !isValidEmail(emailRaw);

  const phoneInvalid = formData.phone.trim() !== "" && !normalisedPhone;
  ...
  const formInvalid =
    ...
    lastNameMissing ||
    postcodeInvalid ||
    contactMethodInvalid ||
    emailInvalid ||
    phoneInvalid ||
    pronounsOtherMissing;
          ...
          <Stack spacing={{ xs: 2.5, sm: 3 }}>
            ...
            <Stack spacing={{ xs: 1.5, sm: 2 }}>
              ...
              <WithTTS copy={COPY.contactDetails} titleVariant="subtitle1">
                <Stack spacing={{ xs: 1.5, sm: 2 }}>
                  <Alert severity="info" variant="outlined" sx={outlinedInfoAlertSx}>
